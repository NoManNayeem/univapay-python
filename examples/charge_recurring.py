...
def main():
    ap = argparse.ArgumentParser(description="Create a charge using a recurring token")
    add_common_args(ap)
    ap.add_argument("--token", required=True, help="Transaction token id (recurring)")
    ...
    idem = ensure_idempotency_key(args.idempotency_key, prefix="recurring")
    print("[RUN] Creating recurring charge")
